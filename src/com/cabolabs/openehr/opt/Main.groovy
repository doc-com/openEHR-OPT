...
class Main {
    ...
    static void main(String[] args) {
        ...
        switch (args[0]) {
            case 'uigen':
                if (args.size() < 2) {
                    println 'usage: opt uigen file_uuid'
                    System.exit(1)
                }
                def path = "tmp" + PS + args[1] + ".opt" //"resources"+ PS +"opts"+ PS +"Encuentro.opt"
                def opt = loadAndParse(path)
                def gen = new OptUiGenerator()
                def ui = gen.generate(opt)
                new File("tmp" + PS + args[1] + ".html") << ui
            ...
            case 'ingen':
                ...
                def path = "tmp" + PS + args[1] + ".opt" //"resources"+ PS +"opts"+ PS +"Referral.opt"
                def opt = loadAndParse(path)
                ...
                */
                ...
                for (i in 1..count) {
                    ...
                    new File("tmp" + PS + args[1] + ".contribution") << ins
                }
                break
            case 'inval':
                ...
                def path = "tmp" + PS + args[1] + ".instance"
                def f = new File(path)
                if (!f.exists()) {
